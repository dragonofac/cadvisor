...
function drawGpuTotalUsage(elementId, machineInfo, stats) {
    if (!hasResource(stats, 'accelectors')) {
        return;
    }
    ...
    for (var i = 1; i < stats.stats.length; i++) {
        ...
        elements.push(cur.gpuAcceleration.usage.total);
        data.push(elements);
    }
    drawLineChart(titles, data, elementId, 'GpuMemory');
}

function drawGpuUtilUsage(elementId, machineInfo, stats) {
    if (!hasResource(stats, 'accelectors')) {
        return;
    }

    var titles = ['Time', 'Util(%)'];
    var data = [];
    for (var i = 0; i < stats.stats.length; i++) {
        ...
        for (var j=0; j < cur.length; j++) {
            var accelector = cur[j];
            avg_util += accelector.duty_cycle;
        }

        var elements = [];
        elements.push(cur.timestamp);
        elements.push(avg_util / cur.length);
        data.push(elements);
    }
    drawLineChart(titles, data, elementId, 'GpuUtil');
}
...
function drawCharts(machineInfo, containerInfo, subcontainers) {
    ...
    // GPU.
    // steps.push(function () {
    //     drawGpuTotalUsage('gpu-total-usage-chart', machineInfo, containerInfo);
    // });

    steps.push(function () {
        drawGpuUtilUsage('gpu-util-usage-chart', machineInfo, containerInfo);
    });
